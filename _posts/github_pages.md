...
While HTML file displaying is straightforward, unless you have a background in styling the pages, it may be overly cumbersome to get a good-looking site. Instead, there are libraries that contain more preset elements that can produce static web pages, including [Jekyll](https://jekyllrb.com/), which integrates easily with GitHub. In the example below, you will clone an existing template site and update the code to represent a simple portfolio site that is also hosted through Pages.
